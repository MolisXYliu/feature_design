...
3. 命令安全检测采用**结构化解析**（PowerShell AST + 参数级校验），与 codex 完全对齐，而非简单的字符串前缀匹配
4. 持久化审批规则仅限构建工具类命令，解释器命令被禁止持久化，平衡了开发体验与安全性
5. 禁止命令在所有模式下（包括 YOLO）都被拦截
6. 审批通道经过多层安全加固（窗口绑定、tool_call_id 匹配、超时机制），防止伪造和重放攻击
7. 首次启动强制引导用户选择 Elevated 模式，降低因配置不当导致的安全风险
